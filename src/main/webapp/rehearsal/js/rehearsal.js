$(function() {
	...
	$("#create_rh").button().click(function() {
		...
		loadpopupform("users/rehearsal/action?create=");
	});
	
	$("#edit_rh").button().click(function() {
		edit("users/rehearsal/action?edit=&id=");		
	});
	
	$("#delete_rh").button().click(function() {
		remove('users/rehearsal/action?remove=&id=');
	});
...
function populateDialog() {
	...
	$("#dialog-form").dialog({
		...
		buttons : {
			"Save" : function() {
				if (validateForm()) {
					save('users/rehearsal/action?save=', 'frm');
					$(this).dialog("close");
				}
			},
			Cancel : function() {
...
function search() {
	ajaxCallsWithPaging('users/rehearsal/action?search=', 'post',
