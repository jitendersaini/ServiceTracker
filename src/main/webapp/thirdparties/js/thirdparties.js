...
local={		
		functions:{
					...
					buttons : {
						"Save" : function() {
							if (local.functions.validateForm()) {
								global.functions.save('users/thirdparties/action?save=', 'frm');
								$(this).dialog("close");
							}
						},
						Cancel : function() {
			...
			search: function() {
				/*global.functions.ajaxCallsWithPaging('users/thirdparties/action?search=', 'post',
						'jtable', 'thirdpartiestable', 'Loading Details', 'Something Went Wrong');*/
				$('#jtable').html(global.functions.tableTemplate());
				global.functions.checkUncheckAll();
				global.functions.loadPaging("users/thirdparties/action?search=");
			}/*,
			loadPaging:function() {
				$(document).ready(function() {
					$('#thirdpartiestable').dataTable({
						"bJQueryUI" : true,
...
$(function() {
	$("#button_actions_tp button:first").button({
		icons : {
	...
	$("#create_tp").button().click(function() {
		
		//alert($('#tabs ul').find('.ui-tabs-active').index());		
		global.functions.loadpopupform("users/thirdparties/action?create=");
	});
	
	$("#edit_tp").button().click(function() {
		local.functions.edit("users/thirdparties/action?edit=&id=");		
	});
	
	$("#delete_tp").button().click(function() {
		global.functions.remove('users/thirdparties/action?remove=&id=');
	});
