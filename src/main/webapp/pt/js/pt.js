					...
					buttons : {
						"Save" : function() {
							if (local.functions.validateForm()) {
								global.functions.save('users/processtools/action?save=', 'frm');
								$(this).dialog("close");
							}
						},
						Cancel : function() {
...
$(function() {
	$("#button_actions_pt button:first").button({
		icons : {
	...
	$("#create_pt").button().click(function() {
		
		//alert($('#tabs ul').find('.ui-tabs-active').index());		
		global.functions.loadpopupform("users/processtools/action?create=");
	});
	
	$("#edit_pt").button().click(function() {
		local.functions.edit("users/processtools/action?edit=&id=");		
	});
	
	$("#delete_pt").button().click(function() {
		global.functions.remove('users/processtools/action?remove=&id=');
	});
