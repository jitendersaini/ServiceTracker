...
local={		
		functions:{
					...
					buttons : {
						"Save" : function() {
							if (local.functions.validateForm()) {
								global.functions.save('users/testing/action?save=', 'frm');
								$(this).dialog("close");
							}
						},
						Cancel : function() {
			...
			search: function() {
				/*global.functions.ajaxCallsWithPaging('users/testing/action?search=', 'post',
						'jtable', 'testingtable', 'Loading Details', 'Something Went Wrong');*/
				$('#jtable').html(global.functions.tableTemplate());
				global.functions.checkUncheckAll();
				global.functions.loadPaging("users/testing/action?search=");
			}
		}
};

$(function() {
	$("#button_actions_testing button:first").button({
		icons : {
	...
	$("#create_testing").button().click(function() {
		
		//alert($('#tabs ul').find('.ui-tabs-active').index());		
		global.functions.loadpopupform("users/testing/action?create=");
	});
	
	$("#edit_testing").button().click(function() {
		local.functions.edit("users/testing/action?edit=&id=");		
	});
	
	$("#delete_testing").button().click(function() {
		global.functions.remove('users/testing/action?remove=&id=');
	});
