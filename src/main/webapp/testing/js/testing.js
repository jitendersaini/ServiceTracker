$(function() {
	...
	$("#create_testing").button().click(function() {
		...
		loadpopupform("users/testing/action?create=");
	});
	
	$("#edit_testing").button().click(function() {
		edit("users/testing/action?edit=&id=");		
	});
	
	$("#delete_testing").button().click(function() {
		remove('users/testing/action?remove=&id=');
	});
...
function populateDialog() {
	...
	$("#dialog-form").dialog({
		...
		buttons : {
			"Save" : function() {
				if (validateForm()) {
					save('users/testing/action?save=', 'frm');
					$(this).dialog("close");
				}
			},
			Cancel : function() {
...
function search() {
	ajaxCallsWithPaging('users/testing/action?search=', 'post',
