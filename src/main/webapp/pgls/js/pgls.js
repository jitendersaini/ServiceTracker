$(function() {
	...
	$("#create_pgls").button().click(function() {
		...
		loadpopupform("users/pgls/action?create=");
	});
	
	$("#edit_pgls").button().click(function() {
		edit("users/pgls/action?edit=&id=");		
	});
	
	$("#delete_pgls").button().click(function() {
		remove('users/pgls/action?remove=&id=');
	});
...
function populateDialog() {
	...
	$("#dialog-form").dialog({
		...
		buttons : {
			"Save" : function() {
				if (validateForm()) {
					save('users/pgls/action?save=', 'frm');
					$(this).dialog("close");
				}
			},
			Cancel : function() {
...
function search() {
	ajaxCallsWithPaging('users/pgls/action?search=', 'post',
