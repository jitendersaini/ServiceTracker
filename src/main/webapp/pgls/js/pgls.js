...
local={		
		functions:{
					...
					buttons : {
						"Save" : function() {
							if (local.functions.validateForm()) {
								global.functions.save('users/pgls/action?save=', 'frm');
								$(this).dialog("close");
							}
						},
						Cancel : function() {
			...
			search: function() {
				/*global.functions.ajaxCallsWithPaging('users/pgls/action?search=', 'post',
						'jtable', 'pglstable', 'Loading Details', 'Something Went Wrong');*/
				$('#jtable').html(global.functions.tableTemplate());
				global.functions.checkUncheckAll();
				global.functions.loadPaging("users/pgls/action?search=");
			}/*,
			loadPaging:function() {
...
$(function() {
	$("#button_actions_pgls button:first").button({
		icons : {
	...
	$("#create_pgls").button().click(function() {
		
		//alert($('#tabs ul').find('.ui-tabs-active').index());		
		global.functions.loadpopupform("users/pgls/action?create=");
	});
	
	$("#edit_pgls").button().click(function() {
		local.functions.edit("users/pgls/action?edit=&id=");		
	});
	
	$("#delete_pgls").button().click(function() {
		global.functions.remove('users/pgls/action?remove=&id=');
	});
