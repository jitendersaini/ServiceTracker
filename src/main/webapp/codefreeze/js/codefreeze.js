...
local={		
		functions:{
					...
					buttons : {
						"Save" : function() {
							if (local.functions.validateForm()) {
								global.functions.save('users/codefreeze/action?save=', 'frm');
								$(this).dialog("close");
							}
						},
						Cancel : function() {
			...
			search: function() {
				/*global.functions.ajaxCallsWithPaging('users/codefreeze/action?search=', 'post',
						'jtable', 'codetable', 'Loading Details', 'Something Went Wrong');*/
				$('#jtable').html(global.functions.tableTemplate());
				global.functions.checkUncheckAll();
				global.functions.loadPaging("users/codefreeze/action?search=");
			}
		}
};
...
$(function() {
	...
	$("#create_cf").button().click(function() {
		...
		global.functions.loadpopupform("users/codefreeze/action?create=");
	});
	
	$("#edit_cf").button().click(function() {
		local.functions.edit("users/codefreeze/action?edit=&id=");		
	});
	
	$("#delete_cf").button().click(function() {
		global.functions.remove('users/codefreeze/action?remove=&id=');
	});
