...
local={		
		functions:{
					...
					buttons : {
						"Save" : function() {
							if (local.functions.validateForm()) {
								global.functions.save('users/kt/action?save=', 'frm');
								$(this).dialog("close");
							}
						},
						Cancel : function() {
			...
			search: function() {				
				$('#jtable').html(global.functions.tableTemplate3());
				global.functions.checkUncheckAll();
				global.functions.loadPaging3("users/kt/action?search=");
			}
		}
};
...
$(function() {
	...
	$("#create_kt").button().click(function() {
		...
		global.functions.loadpopupform("users/kt/action?create=");
	});
	
	$("#edit_kt").button().click(function() {
		local.functions.edit("users/kt/action?edit=&id=");		
	});
