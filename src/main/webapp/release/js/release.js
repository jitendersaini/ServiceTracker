$(function() {
	...
	$("#create_release").button().click(function() {
		...
		loadpopupform("users/release/action?create=");
	});
	
	$("#edit_release").button().click(function() {
		edit("users/release/action?edit=&id=");		
	});
	
	$("#delete_release").button().click(function() {
		remove('users/release/action?remove=&id=');
	});
...
function populateDialog() {
	...
	$("#dialog-form").dialog({
		...
		buttons : {
			"Save" : function() {
				if (validateForm()) {
					save('users/release/action?save=', 'frm');
					$(this).dialog("close");
				}
			},
			Cancel : function() {
...
function search() {
	ajaxCallsWithPaging('users/release/action?search=', 'post',
