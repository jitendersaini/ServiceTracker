					...
					buttons : {
						"Save" : function() {
							if (local.functions.validateForm()) {
								global.functions.save('users/release/action?save=', 'frm');
								$(this).dialog("close");
							}
						},
						Cancel : function() {
...
$(function() {
	$("#button_actions_release button:first").button({
		icons : {
	...
	$("#create_release").button().click(function() {
		
		//alert($('#tabs ul').find('.ui-tabs-active').index());		
		global.functions.loadpopupform("users/release/action?create=");
	});
	
	$("#edit_release").button().click(function() {
		local.functions.edit("users/release/action?edit=&id=");		
	});
	
	$("#delete_release").button().click(function() {
		global.functions.remove('users/release/action?remove=&id=');
	});
